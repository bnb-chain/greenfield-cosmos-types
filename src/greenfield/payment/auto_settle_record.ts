...
export interface AutoSettleRecordProtoMsg {
...
export const AutoSettleRecord = {
  ...
  toAmino(message: AutoSettleRecord): AutoSettleRecordAmino {
    const obj: any = {};
    obj.timestamp = message.timestamp ? message.timestamp.toString() : undefined;
    obj.addr = message.addr;
    return obj;
  },
  fromAminoMsg(object: AutoSettleRecordAminoMsg): AutoSettleRecord {
    return AutoSettleRecord.fromAmino(object.value);
  },
  fromProtoMsg(message: AutoSettleRecordProtoMsg): AutoSettleRecord {
    return AutoSettleRecord.decode(message.value);
  },
  toProto(message: AutoSettleRecord): Uint8Array {
    return AutoSettleRecord.encode(message).finish();
  },
  toProtoMsg(message: AutoSettleRecord): AutoSettleRecordProtoMsg {
    return {
      typeUrl: "/greenfield.payment.AutoSettleRecord",
      value: AutoSettleRecord.encode(message).finish()
    };
  }
};